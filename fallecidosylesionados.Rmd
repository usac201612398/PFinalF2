...
### El arbol no despliega ramas por lo tanto se procedió a analizar por separado los casos (fallecimientos y lesionados) ya que es más la tendencia a lesionados y una muerte ya es una situación crítica. No obstante, hubo la misma situación que el dataset no proporcionó detalles entonces se balancea para dejar casos aleatorios con el mismo numero de casos para fallecidos como lesionados.
...
arbol2 <- rpart(tipo_eve ~  tipo_veh +depto_ocu + fall_les + mayor_menor + horario,
...
escenarios <- data.frame(
  sexo_per  = factor(c(2, 1, 2, 1, 2), levels = levels(data_bal$sexo_per)),   # Mujer/Hombre
  horario  = factor(c(1, 2, 1, 1, 2), levels = levels(data_bal$tipo_eve)),   # Día, Noche
  mayor_menor  = factor(c(1, 2, 2, 1, 1), levels = levels(data_bal$tipo_eve)),   # Mayor,Menor
  tipo_veh  = factor(c(4, 1, 3, 2, 5), levels = levels(data_bal$tipo_veh)),   # Motocicleta, Automóvil, Pick up, Camioneta sport, Camión
  depto_ocu = factor(c(1, 6, 14, 18, 22), levels = levels(data_bal$depto_ocu)), # Guatemala, Santa Rosa, Quiché, Izabal, Jutiapa
  fall_les = factor(c(1, 1, 2, 2, 2), levels = levels(data_bal$depto_ocu)) # Guatemala, Santa Rosa, Quiché, Izabal, Jutiapa
)
