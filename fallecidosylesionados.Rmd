...
# 5 escenarios distintos
escenarios <- data.frame(
  sexo_per  = factor(c(2, 1, 2, 1, 2), levels = levels(data_arbol5$sexo_per)),   # Mujer/Hombre
  mayor_menor  = factor(c(1, 2, 2, 1, 1), levels = levels(data_arbol5$mayor_menor)),   # Mayor,Menor
  tipo_eve  = factor(c(2, 5, 3, 1, 6), levels = levels(data_arbol5$tipo_eve)), # Choque, Atropello, Vuelco, Colisión, Derrape
  tipo_veh  = factor(c(4, 1, 3, 2, 5), levels = levels(data_arbol5$tipo_veh)),   # Motocicleta, Automóvil, Pick up, Camioneta sport, Camión
  
  fall_les = factor(c(1, 1, 2, 2, 2), levels = levels(data_arbol5$depto_ocu)) # Fallecido, Lesionado
)
...
pred_rf1b <- predict(modelo_rf1b, escenarios, type = "prob")
pred_rf1b
...
conf_matrix <- table(test_data$tipo_eve, predictions)
conf_matrix
...
conf_matrix <- table(test_data$tipo_veh, predictions)
conf_matrix
...
conf_matrix <- table(test_data$horario, predictions)
conf_matrix
...
conf_matrix <- table(test_data$sexo_per, predictions)
conf_matrix
...
conf_matrix <- table(test_data$mayor_menor, predictions)
conf_matrix
...
install.packages("writexl")   # Ejecuta solo una vez
library(writexl)

# Si quieres preservar etiquetas de factores:
df_export_xlsx <- datos_consolidados %>% dplyr::mutate(dplyr::across(where(is.factor), as.character))

# Exportar a Excel
write_xlsx(df_export_xlsx, path = "datos_consolidados.xlsx")
